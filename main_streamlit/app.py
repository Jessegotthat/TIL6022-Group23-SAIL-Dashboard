...
def build_heatmap_frames_global(flow_long: pd.DataFrame, sensors: pd.DataFrame, times):
    ...
    ref = float(np.percentile(all_vals, 99))
    ref = max(ref, 1.0)
...
def configure_page():
    # Guard so the combined host can also set page config
    try:
        st.set_page_config(page_title="SAIL Amsterdam 2025 — Crowd Heatmap Dashboard", layout="wide")
    except Exception:
        pass
...
def run():
    ...
    if ss.page == "timelapse":
        ...
        if timelapse_mode.startswith("Heatmap"):
            ...
            gradient = {
                0.00: '#0000ff', 0.25: '#33a3ff', 0.50: '#33ff57',
                0.75: '#ffd43b', 1.00: '#ff3b30',
            }
            HeatMapWithTime(
                frames,
            ...
            # Bubbles with TimestampedGeoJson
            ...
            for t in frame_times:
                ...
                for _, r in dt.iterrows():
                    ...
                    features.append({
                        "type": "Feature",
                        "geometry": {"type": "Point", "coordinates": [float(r["_lon"]), float(r["_lat"])]},
                        "properties": {
                            "time": t.isoformat(),
                            "popup": f"{r['location_name']}<br><b>Count:</b> {int(val)}<br><b>Time:</b> {t:%Y-%m-%d %H:%M}",
                            "style": {"color": color, "fillColor": color, "fillOpacity": 0.85, "opacity": 0.95},
                            "icon": "circle",
                            "iconstyle": {"fillColor": color, "fillOpacity": 0.85, "stroke": True, "radius": rad},
                        },
                    })
        ...
        if not locations:
            ...
            if detail_df.empty:
                ...
                <div style="display:flex;flex-direction:column;align-items:flex-start;margin-top:1rem;">
                  <div style="font-size:2rem;font-weight:600;color:white;">{now_val:,.2f}</div>
                  <div style="color:{delta_color};background-color:{delta_color}20;
                              padding:.25rem .6rem;border-radius:8px;font-size:.9rem;
                              font-weight:500;margin-top:.3rem;">
                    {'▲' if delta_val>=0 else '▼'} {delta_val:,.2f} vs 24 h avg
